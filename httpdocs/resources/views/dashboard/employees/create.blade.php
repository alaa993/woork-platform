@extends('layouts.app')
@section('title', __('woork.create_employee'))
@section('page', __('woork.create_employee'))
@section('content')
<div class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur p-5">
  ...
  <form method="POST" action="{{ route('employees.store') }}" class="grid md:grid-cols-2 gap-4">@csrf
    ...
    <div>
      ...
      <input name="title" value="{{ old('title', '') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>

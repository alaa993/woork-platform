@extends('layouts.app')
@section('title', __('woork.edit_camera'))
@section('page', __('woork.edit_camera'))
@section('content')
<div class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur p-5">
  ...
  <form method="POST" action="{{ route('cameras.update',$camera->id) }}" class="grid md:grid-cols-2 gap-4">@csrf @method('PUT')
    ...
    <div>
      ...
      <select name="room_id" required class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
        ...
        @foreach($rooms as $room)
          <option value="{{ $room->id }}" @selected(old('room_id', $camera->room_id) == $room->id)>{{ $room->name }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div>
      ...
      <select name="agent_device_id" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
        ...
        @foreach($agentDevices as $device)
          <option value="{{ $device->id }}" @selected(old('agent_device_id', $camera->agent_device_id) == $device->id)>{{ $device->name }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div>
      ...
      <input name="name" value="{{ old('name', $camera->name ?? '') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>

    <div>
      <label class="block text-sm mb-1">{{ __('woork.purpose') }}</label>
      <input name="purpose" value="{{ old('purpose', $camera->purpose ?? 'desk') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>
    ...
    <div>
      ...
      <select name="analysis_mode" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
        @foreach($analysisModes as $mode => $profile)
          <option value="{{ $mode }}" @selected(old('analysis_mode', $camera->analysis_mode ?? $defaultMode) === $mode)>{{ $profile['label'] }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div>
      ...
      <input name="rtsp_url" value="{{ old('rtsp_url', $camera->rtsp_url ?? '') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    ...
    <div>
      ...
      <input name="status" value="{{ old('status', $camera->status ?? 'pending') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>

    <div>
      <label class="block text-sm mb-1">{{ __('woork.stream_status') }}</label>
      <input name="stream_status" value="{{ old('stream_status', $camera->stream_status ?? '') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>

    <div>
      <label class="block text-sm mb-1">{{ __('woork.health_message') }}</label>
      <input name="health_message" value="{{ old('health_message', $camera->health_message ?? '') }}" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">
    </div>

    <div class="md:col-span-2">
      <label class="block text-sm mb-1">{{ __('woork.roi') }}</label>
      <textarea name="roi" rows="4" class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-800 placeholder-slate-400 shadow-sm focus:border-emerald-500 focus:ring-emerald-300 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700">{{ old('roi', $camera->roi ?: $defaultRoiJson) }}</textarea>
